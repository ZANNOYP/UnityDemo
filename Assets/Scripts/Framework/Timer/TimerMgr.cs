...
public class TimerMgr : BaseManager<TimerMgr>
{
    ...
    {
        float lastTime = isRealTime ? Time.realtimeSinceStartup : Time.time;
        while (true)
        {
            //100毫秒进行一次计时
            if (isRealTime)
            ...
            float now = isRealTime ? Time.realtimeSinceStartup : Time.time;
