# UnityDemo

## 项目简介
这是一个基于Unity引擎开发的简单Demo，包含以下功能：
- 角色控制：实现人物移动、跳跃与攻击，
- 怪物AI：巡逻与索敌行为，
- 交互：人物与血包、钥匙、武器和门的互动，人物拾取掉落物。

## 项目功能
1. **角色控制**  
   实现了基本的角色控制系统，包括：
   - 人物在地面上自由移动。
   - 按下空格键进行跳跃。
   - 鼠标左键进行攻击。

2. **怪物AI**  
   - 怪物会在一定区域内巡逻。
   - 当玩家进入怪物的索敌范围时，怪物会追踪玩家。
   - 进入攻击范围后攻击玩家。
   - 玩家跑离一定范围，怪物自动回到巡逻点继续巡逻。

3. **交互**  
   - 角色接近门时，显示交互提示信息，比如“E开门”，按下E键打开背包可以使用钥匙开门。
   - 角色靠近钥匙、武器、血包时，显示交互提示信息，比如“E拾取”，按下E拾取钥匙放入背包中

4. **血条**  
   - 玩家血条显示在左下角，怪物血条显示在头顶。
   - 每当玩家攻击怪物，怪物血条会显示几秒后隐藏。

5. **菜单**  
   - 主菜单开始游戏、退出游戏
   - 游戏界面ESC暂停游戏
   - 设置界面调节音量、音效
   - 背包查看以获取物品

6. **游戏结束**  
   - 被怪物击杀或进入锁住的门后的通关点

## 技术栈
- **Unity**：使用 Unity 引擎进行开发，包含物理引擎、动画控制、UI显示、NavMesh导航系统等。
- **C#**：使用 C# 编写脚本，控制游戏逻辑和互动。

## 使用说明
- 移动角色：使用WASD控制角色移动，空格键跳跃，左Shift键奔跑，鼠标左键攻击。
- 交互：E键
- 暂停：ESC键暂停。
- 设置：暂停时按设置键。
- 背包：暂停时按背包键

## TODO
- 添加更多UI功能，比如角色属性。
- 添加更多场景，比如不同房间。
